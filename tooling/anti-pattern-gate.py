#!/usr/bin/env python3
"""
anti-pattern-gate hook (generic vibe-fork production-code discipline).
...
rejects the lazy escape hatches subagents reach for when they're stuck:
  ...
  - Arc<Mutex<T>>            - escape hatch from ownership design (Rust)
  - Rc<RefCell<T>>           - same (single-threaded Rust)
  - module-root #![allow]    - root-level lint silencing
  - silent boundary round-trip - same-expr cross-substrate ping-pong
...
PROJECT CUSTOMIZATION:
  ...
  - Edit the path-predicate to match your gated tree.
  - Edit the test-block detector if your target language uses different
    syntax for test markers.
"""
...
PATTERNS = [
    ...
    (
        ...
        "Propagate the error via `Result<T, ProjectError>`. The error "
    ...
    (
        ...
        "Propagate the error via `Result<T, ProjectError>`.",
    ),
    (
        ...
        "R-CODE-2).",
        "Propagate via `Result<T, ProjectError>`. If this is a true "
    ...
    # Add project-specific boundary round-trip patterns here. Example
    # for a GPU-aware codebase:
    # (
    #     re.compile(r"\.\s*cpu\s*\(\s*\)[^;\n]*\.\s*(?:cuda|to_device)\s*\("),
    #     "silent CPU->GPU round trip",
    #     "A tensor was downloaded to CPU and immediately re-uploaded "
    #     "to GPU in the same expression. Data should never have left "
    #     "the GPU. (goal.md R-CODE-4)",
    #     "Remove the .cpu call. If the intermediate code path requires "
    #     "CPU, audit the call chain to find the actual GPU-side op "
    #     "that should exist.",
    # ),
]
