// ferrum: Thread pool configuration and parallel thresholds (REQ-6, REQ-7, REQ-8)

use std::sync::OnceLock;

use rayon::ThreadPool;

/// Global ferrum thread pool, initialized once.
...
pub fn set_num_threads(n: usize) -> Result<(), String> {
    ...
    GLOBAL_POOL
        .set(pool)
        .map_err(|_| "ferrum thread pool already initialized".to_string())
}
