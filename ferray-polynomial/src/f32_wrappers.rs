...
//! ## REQ status
//!
//! Internal f32-boundary infrastructure — provides the f32 sibling
//! newtypes for the non-power polynomial bases (REQ-2 `Chebyshev`,
//! REQ-5 `Hermite`, REQ-6 `HermiteE`, REQ-4 `Laguerre`, REQ-3
//! `Legendre`). No standalone numpy REQ of its own; it widens the
//! coefficient dtype surface of those classes to `f32`.
//!
//! - SHIPPED — `ChebyshevF32`/`HermiteF32`/`HermiteEF32`/`LaguerreF32`/
//!   `LegendreF32` newtypes generated by the `impl_f32_wrapper!` macro;
//!   each delegates eval/deriv/integ/roots/fit to its f64 sibling
//!   (`Chebyshev`/`Hermite`/`HermiteE`/`Laguerre`/`Legendre`) via the
//!   `Poly` trait, promoting on `new` and demoting on observation.
//!   Consumer: re-exported from `crate::lib` and bound by the
//!   `poly_class!` shims in `ferray-python/src/polynomial.rs` that
//!   accept f32 coefficient arrays.
//! - NOT-STARTED — none (the f32-native `Poly<T>` generic path #731 is
//!   a performance alternative, not a missing REQ; the wrapper path is
...
macro_rules! impl_f32_wrapper {
    ($wrapper:ident, $f64ty:ty) => {
        /// f32 sibling of the corresponding f64 polynomial basis type.
        /// Wraps the f64 type and promotes/demotes on the public
        /// boundary. See module-level docs for the tradeoffs vs. the
        /// planned generic path (#731).
        #[derive(Debug, Clone)]
        pub struct $wrapper {
            inner: $f64ty,
        }
