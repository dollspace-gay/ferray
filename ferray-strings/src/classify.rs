...
//
// ## REQ status
// All classification predicates below match CPython `str.is*()` element
// by element, which is exactly what `numpy.strings`/`numpy.char` delegate
// to. The non-ASCII / category-sensitive predicates (`isalpha`,
// `isdecimal`, `isdigit`, `isupper`, `islower`, `istitle`, `isspace`)
// are backed by sorted, non-overlapping codepoint-range tables derived
// LIVE from the oracle CPython 3.13 / Unicode 15.1.0 (the version backing
...
/// Binary-search a sorted, non-overlapping list of inclusive codepoint
/// ranges for `c`.
...
/// Inclusive codepoint ranges of characters with Unicode
/// `Numeric_Type` of Decimal — i.e. the set for which CPython's
/// `str.isdecimal()` returns `True` for a single character
/// (`Py_UNICODE_ISDECIMAL`). This is the Decimal-only subset of
/// [`DIGIT_RANGES`]: it drops superscripts/subscripts (`²³⁴`), circled
/// (`①`) and parenthesized (`⑴`) digits, keeping only true positional
/// decimal digits (ASCII `0-9`, fullwidth `０-９`, Arabic-Indic `٠-٩`,
/// Devanagari, etc.).
///
/// Derived live from the oracle CPython 3.13 / Unicode 15.1.0:
/// `[c for c in range(0x110000) if chr(c).isdecimal()]`, run-length
/// compressed to 64 ranges.
...
/// Inclusive codepoint ranges of characters in Unicode general category
/// `L*` (`Lu | Ll | Lt | Lm | Lo`), i.e. exactly the set for which
/// CPython's `str.isalpha()` returns `True` for a single character. This
/// is NARROWER than Rust's `char::is_alphabetic` (the `Alphabetic`
/// derived property, which also includes combining marks `Mn`, letter-
/// numbers `Nl` such as roman numerals `Ⅻ`, etc.).
///
/// Derived live from the oracle CPython 3.13 / Unicode 15.1.0:
/// `[c for c in range(0x110000) if chr(c).isalpha()]`, run-length
/// compressed to 660 ranges.
const ALPHA_RANGES: &[(char, char)] = &[
    ('\u{0041}', '\u{005A}'),
    ('\u{0061}', '\u{007A}'),
    ('\u{00AA}', '\u{00AA}'),
    ('\u{00B5}', '\u{00B5}'),
...
/// Inclusive codepoint ranges of characters with the Unicode `Uppercase`
/// property (general category `Lu` plus `Other_Uppercase`, e.g. roman
/// numerals `Ⅻ` and circled letters `Ⓐ`). Mirrors CPython's
/// `Py_UNICODE_ISUPPER`. Titlecase (`Lt`) characters are excluded (they
/// live in [`TITLE_RANGES`]).
///
/// Derived live from the oracle CPython 3.13 / Unicode 15.1.0,
/// run-length compressed to 651 ranges.
const UPPER_RANGES: &[(char, char)] = &[
    ('\u{0041}', '\u{005A}'),
    ('\u{00C0}', '\u{00D6}'),
    ('\u{00D8}', '\u{00DE}'),
    ('\u{0100}', '\u{0100}'),
...
/// Inclusive codepoint ranges of characters with the Unicode `Lowercase`
/// property (general category `Ll` plus `Other_Lowercase`, e.g. roman
/// numerals `ⅻ`). Mirrors CPython's `Py_UNICODE_ISLOWER`.
///
/// Derived live from the oracle CPython 3.13 / Unicode 15.1.0,
/// run-length compressed to 671 ranges.
const LOWER_RANGES: &[(char, char)] = &[
    ('\u{0061}', '\u{007A}'),
    ('\u{00AA}', '\u{00AA}'),
    ('\u{00B5}', '\u{00B5}'),
    ('\u{00BA}', '\u{00BA}'),
...
/// Inclusive codepoint ranges of characters in Unicode general category
/// `Lt` (titlecase letters such as `ǅ`, `ǈ`, `ǋ`, `ǲ`). Mirrors
/// CPython's `Py_UNICODE_ISTITLE`. These count as neither upper nor
/// lower but ARE cased, and open a title-word.
///
/// Derived live from the oracle CPython 3.13 / Unicode 15.1.0.
const TITLE_RANGES: &[(char, char)] = &[
    ('\u{01C5}', '\u{01C5}'),
    ('\u{01C8}', '\u{01C8}'),
    ('\u{01CB}', '\u{01CB}'),
    ('\u{01F2}', '\u{01F2}'),
...
/// Inclusive codepoint ranges of characters for which CPython's
/// `str.isspace()` returns `True` (Unicode `White_Space` plus the bidi
/// `WS`/`B`/`S` C0 controls `U+001C`..`U+001F`). BROADER than Rust's
/// `char::is_whitespace` (the `White_Space` property only), which omits
/// the file/group/record/unit separators.
///
/// Derived live from the oracle CPython 3.13 / Unicode 15.1.0.
...
fn is_alpha_char(c: char) -> bool {
    in_ranges(ALPHA_RANGES, c)
}

/// `true` if `c` has the Unicode `Uppercase` property (`Py_UNICODE_ISUPPER`).
fn is_upper_char(c: char) -> bool {
    in_ranges(UPPER_RANGES, c)
}

/// `true` if `c` has the Unicode `Lowercase` property (`Py_UNICODE_ISLOWER`).
fn is_lower_char(c: char) -> bool {
    in_ranges(LOWER_RANGES, c)
}

/// `true` if `c` is a titlecase letter, category `Lt` (`Py_UNICODE_ISTITLE`).
fn is_title_char(c: char) -> bool {
    in_ranges(TITLE_RANGES, c)
}

/// `true` if `c` is whitespace per `str.isspace` (`Py_UNICODE_ISSPACE`).
fn is_space_char(c: char) -> bool {
    in_ranges(WHITESPACE_RANGES, c)
}
...
pub fn isalpha<D: Dimension>(a: &StringArray<D>) -> FerrayResult<Array<bool, D>> {
    classify(a, |s| !s.is_empty() && s.chars().all(is_alpha_char))
}
...
pub fn isspace<D: Dimension>(a: &StringArray<D>) -> FerrayResult<Array<bool, D>> {
    classify(a, |s| !s.is_empty() && s.chars().all(is_space_char))
}
...
pub fn isupper<D: Dimension>(a: &StringArray<D>) -> FerrayResult<Array<bool, D>> {
    classify(a, |s| {
        let mut cased = false;
        for c in s.chars() {
            if is_lower_char(c) || is_title_char(c) {
                return false;
            }
            if is_upper_char(c) {
                cased = true;
...
pub fn islower<D: Dimension>(a: &StringArray<D>) -> FerrayResult<Array<bool, D>> {
    classify(a, |s| {
        let mut cased = false;
        for c in s.chars() {
            if is_upper_char(c) || is_title_char(c) {
                return false;
            }
            if is_lower_char(c) {
                cased = true;
...
/// (`Lt`) characters such as `ǅ` count as an opening (uppercase-position)
/// character, so `"ǅ"` and `"ǅx"` are titlecased.
...
mod tests {
    ...
    // --- Divergence pins for crosslink #823/#911 (isnumeric/isdecimal/
    // isalnum/isdigit). Expected values are the live results of CPython
    // 3.13 / numpy 2.4 (Unicode 15.1.0) per-character `str.is*()`. ---
    ...
    // --- Divergence pins for crosslink #912 (refs #836). Expected values
    // are the live results of CPython 3.13 / numpy.char (Unicode 15.1.0),
    // confirmed against `numpy.char.is*` element by element. ---

    // isdecimal: `Numeric_Type=Decimal`. Non-ASCII positional decimals
    // (fullwidth '０', Arabic-Indic '٣') are True; superscript '²' and
    // fraction '½' are False; ASCII '123' True. The PRE-FIX impl used
    // `is_ascii_digit()` and returned False for '０'/'٣' (the divergence).
    #[test]
    fn test_isdecimal_python_semantics_912() {
        let a = array(&["０", "٣", "²", "½", "①", "123", "12.3", ""]).unwrap();
    ...
    // isalpha: Unicode general category L* ONLY. Roman numeral 'Ⅻ' (Nl)
    // and circled 'Ⓐ' (So) are NOT alpha; accented 'café', CJK '日本' and
    // titlecase digraph 'ǅ' (Lt) ARE. The PRE-FIX impl used
    // `char::is_alphabetic` (the Alphabetic property) and returned True
    // for 'Ⅻ' (the divergence).
    #[test]
    fn test_isalpha_python_semantics_912() {
        let a = array(&["café", "日本", "ǅ", "Ⅻ", "Ⓐ", "abc123", "123", ""]).unwrap();
        let r = isalpha(&a).unwrap();
        assert_eq!(
            r.as_slice().unwrap(),
    ...
    // istitle: titlecase Lt char counts as an opening (uppercase-position)
    // character. 'ǅ' and 'ǅx' are titlecased; 'ǅX' is not (two cased in a
    // row). 'Ⅻ' (Nl, Uppercase property) opens a title word -> True. The
    // PRE-FIX impl used `char::is_uppercase` and missed Lt (returned False
    // for 'ǅ', the divergence).
    #[test]
    fn test_istitle_python_semantics_912() {
        let a = array(&["Hello", "ǅ", "ǅx", "ǅX", "Ⅻ", "hELLO", "Hello World", ""]).unwrap();
        let r = istitle(&a).unwrap();
        assert_eq!(
            r.as_slice().unwrap(),
    ...
    // isupper/islower: Unicode Uppercase/Lowercase properties (incl.
    // Other_Uppercase/Lowercase). Roman 'Ⅻ' (Nl, Uppercase prop) is upper;
    // 'ⅻ' (lowercase prop) is lower; titlecase 'ǅ' is neither. Digraph
    // 'ǆ' (Ll) is lower.
    #[test]
    fn test_isupper_islower_python_semantics_912() {
        let up = array(&["ABC", "Ⅻ", "Ǆ", "aBc", "ǅ", "A1", "123", ""]).unwrap();
        assert_eq!(
            isupper(&up).unwrap().as_slice().unwrap(),
            &[true, true, true, false, false, true, false, false]
        );
        let lo = array(&["abc", "ⅻ", "ǆ", "Hello", "ǅ", "a1", "café", ""]).unwrap();
        assert_eq!(
            islower(&lo).unwrap().as_slice().unwrap(),
            &[true, true, true, false, false, true, true, false]
        );
    }

    // isalnum composes the corrected predicates: alpha OR decimal OR digit
    // OR numeric. '½' (numeric) and '②' (digit) are alnum; whitespace is
    // not. 'Ⅻ' is numeric -> alnum.
    #[test]
    fn test_isalnum_python_semantics_912() {
        let a = array(&["a1", "②", "½", "Ⅻ", "café", "ab c", "abc", ""]).unwrap();
        let r = isalnum(&a).unwrap();
        assert_eq!(
            r.as_slice().unwrap(),
    ...
    // isspace: Unicode White_Space + bidi separators U+001C..U+001F. The
    // non-ASCII spaces NBSP (U+00A0) and line/para separator (U+2028/9)
    // are space; ZWSP (U+200B) is NOT.
    #[test]
    fn test_isspace_python_semantics_912() {
        let a = array(&["\u{00A0}", "\u{2028}", "\u{1C}", "\u{200B}", " ", "a b", ""]).unwrap();
        let r = isspace(&a).unwrap();
        assert_eq!(
            r.as_slice().unwrap(),
