...
- Blocker for REQ-3 of ferray-core-string: string comparison (U/S) lexicographic -> bool — less/greater/less_equal/greater_equal/equal/not_equal on string arrays. Add an is_string_compare gate (both operands dtype.kind in {U,S}) ahead of the real-only comparison_body!/complex paths in the bind_comparison! macro (ufunc.rs), delegate to numpy's np.<op>(x1,x2) on the ORIGINAL operands (bool output, codepoint-lexicographic, broadcast). Mirror the datetime is_time_compare seam at ufunc.rs ~2258. U vs S mixed-kind follows numpy's own error/coercion. (#961)
- Blocker for REQ-2 of ferray-core-string: manipulation passthrough preserving string (U/S) dtype — reshape/ravel/transpose/swapaxes/moveaxis/flip/roll/repeat/tile/squeeze/expand_dims/concatenate/stack/where/unique/sort/argsort. Add an is_string_array gate (dtype.kind in {U,S}) ahead of each real-only match_dtype_all_complex!/ordering macro, delegate to numpy's own op (preserves dtype+width, concatenate width-promotes to max), return the numpy string result. Reuse the #948 datetime delegate-passthrough seam (delegate_manip/delegate_manip_list), but for strings the result is returned directly (no int64-view roundtrip). (#960)
- Blocker for REQ-1 of ferray-core-string: detect-and-delegate string (U/S) input coercion in array/asarray/zeros/ones/empty/full/*_like — branch on dtype.kind in {U,S} (or str-list input) ahead of match_dtype_all!/creation_dispatch!, delegate construction to np.asarray(obj,dtype)/np.zeros(shape,dtype) and return the numpy string ndarray unchanged (no int64-view transport needed). Mirror datetime #941. (#959)
- Blocker for REQ-5 of ferray-core-float16: fr.less/greater/equal/... and data-move ops (reshape/where/sort/concatenate/...) reject float16 — needs float16 arms in match_dtype_orderable!/match_dtype_all! (compares via f32-promote bool; data-move via astype-narrow round-trip) (binding, ufunc.rs/manipulation.rs/conv.rs) (#955)
- Blocker for REQ-3 of ferray-core-float16: fr.add/subtract/multiply/divide/power/floor_divide/remainder/maximum/minimum reject float16 — needs a promote-narrow wrapper on binary_numeric_body!/binary_numeric_split_body! (f32-compute, f16 astype-narrow), NEP-50 via numpy result_type (binding, ufunc.rs) (#953)
- Blocker for REQ-4 of ferray-core-float16: fr.sum/prod reject float16 and fr.mean/std/var silently WIDEN float16->float64 (numpy returns float16) — needs float16 arms in stats.rs computing at f32 and narrowing the reduced result via astype, stop coercing mean/std/var output to float64 (binding, stats.rs) (#954)
- Blocker for REQ-1 of ferray-core-float16: fr.array/asarray/full(dtype='float16') reject float16 — needs a float16 branch ahead of match_dtype_all!/full_impl routing through np.asarray(obj,'float16') (binding, conv.rs/creation.rs) (#952)
- Divergence (HIGH): diff/ediff1d/abs/negative/sign/maximum/minimum/where/clip reject datetime/timedelta (numpy computes) (#947)
- Divergence (HIGH): manipulation ops reject datetime64 instead of preserving dtype (#948)
- Divergence (R-CODE-4 CRITICAL): datetime/timedelta reductions silently return float64 instead of datetime64/timedelta64 (#946)
- Blocker for REQ-5 of ferray-core-datetime: fr.arange(datetime strings, dtype=datetime64[M]) -> 'must be real number, not str'; arange coerces bounds to f64. Datetime arange binding branch (epic #940) (#945)
- Blocker for REQ-3 of ferray-core-datetime: fr.less/min/max/argmin/sort/searchsorted over datetime64 reject (real-only dispatch). Needs int64-view compare/order binding arms + NaT-last (epic #940) (#943)
- Blocker for REQ-2 of ferray-core-datetime: timedelta*int / td/td / td//td / td%td reject (no kernel); dt+dt/dt*scalar raise ferray text not numpy UFuncTypeError. Needs 4 small ops/datetime.rs kernels + binding arms (epic #940) (#942)
- Blocker for REQ-4 of ferray-core-datetime: fr.mean(dt) returns float 18268.5 / fr.std(td) returns 0.5 where numpy RAISES; fr.sum(td) rejects where numpy->timedelta. stats.rs M/m arms + raise-numpy-exception guard (epic #940) (#944)
- Blocker for REQ-1 of ferray-core-datetime: fr.array/asarray/zeros/full(dtype=datetime64|timedelta64) reject input — match_dtype_all!/creation_dispatch! lack M/m arms (epic #940) (#941)
- Complex linalg D-class: svd/qr/pinv/matrix_rank/slogdet/matrix_power raise TypeError where numpy computes complex (#939)
- Complex D-class: take/trace/nonzero/kron/tensordot/vander/pad/broadcast_to/count_nonzero/searchsorted/partition/geomspace/eye/identity/intersect1d/union1d/setdiff1d/isin raise TypeError where numpy computes complex (#938)
- R-CODE-4 SILENT-CAST: nanmin/nanmax/nanmedian/nancumsum/nancumprod/meshgrid/nanvar/nanstd/linalg.eig/eigvals drop complex imaginary -> wrong real result (#937)
- Complex compute-where-numpy-RAISES (E): heaviside/copysign/nextafter/hypot/logaddexp/fmod/arctan2 accept complex but numpy rejects (#936)
- Complex misc missing: diff/convolve/logical_and/logical_not/array_equal/linspace/arange(complex dtype) raise on complex (#935)
- Complex linalg-core missing: dot/vdot/inner/outer/matmul/linalg.norm/det/inv/solve raise on complex (numpy computes) (#931)
- Complex ordering-ops missing: sort/argsort/unique/min/max/amin/amax/argmin/argmax/ptp raise on complex (numpy lexicographic) (#932)
- Complex ufuncs missing: square/reciprocal/sign(z/|z|)/isnan/isinf/isfinite raise on complex (numpy registers complex loops) (#934)
- Complex dtype-preserving manipulation missing: reshape/transpose/concatenate/stack/where/flip/roll/repeat/tile raise on complex (#933)
- R-CODE-4 DATA CORRUPTION: complex nan-reductions/median/correlate/round SILENTLY discard imaginary part (#930)
- ferray-ufunc complex branch-cut: arcsin/arccos/arctanh wrong imaginary SIGN at real-axis cut (|re|>1, im==+0) vs numpy C99 (refs #919) (#926)
- Blocker for R-6 of ferray-core-complex: fr.sum/prod/mean over complex. sum/prod -> TypeError (match_dtype_numeric! real-only) though library ReduceAcc for Complex<f32/f64> ALREADY EXISTS (reductions.rs:129/137, Acc=Self). mean -> 1.0 DATA CORRUPTION (match_dtype_float! coerces complex->f64 dropping imag; numpy mean(complex)->complex, c64->c128). Add complex arms to sum/prod/mean dispatch (stats.rs); mean computes complex sum/count -> complex128, mirroring ma #873. (#925)
- Blocker for R-5 of ferray-core-complex: fr.add/subtract/multiply/divide(complex) -> TypeError (top-level ufuncs use match_dtype_numeric!, real-only) though library add_broadcast/subtract/multiply (T:WrappingArith) + divide_broadcast (T:TrueDivide) ALREADY accept Complex (#869). fr.equal/not_equal(complex) must compute (equal_broadcast is PartialEq-bound, OK once dispatched). fr.less/greater/le/ge(complex) numpy computes LEXICOGRAPHICALLY (real,then imag) — does NOT raise (live oracle); less_broadcast is PartialOrd-bound so Complex needs a lexicographic helper (mirror ma #874 cmp_complex_arm!). fr.floor_divide/remainder/bitwise(complex) RAISE TypeError per numpy. Add complex arms to binary_numeric_body/comparison dispatch + lexicographic compare + the raise guards. (#924)
- Divergence: ferray-python rint(complex) raises TypeError but numpy registers a complex loop (TD('fdg'+cmplx)) (#928)
- Divergence: ferray-python sinc(complex) raises TypeError but numpy COMPUTES (sin(pi*x)/(pi*x)) (#927)
- Blocker for R-4 of ferray-core-complex: fr.abs(complex)->TypeError (must be REAL magnitude float64/32 via ferray_ufunc::abs which exists); fr.negative(complex)->TypeError (must negate; library negative is Float-only — bind via subtract(0,z) or a complex negative); positive(complex) already works. Wire the complex arms in bind_unary_numeric_split / absolute. (#923)
- Blocker for R-3 of ferray-core-complex: fr.sqrt/exp/log/sin/cos/tan/sinh/cosh/tanh/arc*/log2/log10/expm1/log1p/power(complex) must compute complex. Library transcendentals ALREADY EXIST (ferray_ufunc::{sin_complex,exp_complex,ln_complex,sqrt_complex,...}, num_complex-backed) — gap is binding dispatch only (a complex arm in the unary-float bind macro + the binary power path). (#922)
- Blocker for R-2 of ferray-core-complex: fr.sqrt/exp/log/sin(complex) SILENTLY DISCARD imaginary part (ComplexWarning, real-only result = DATA CORRUPTION R-CODE-4). Root cause: unary_float_body! (ufunc.rs) routes complex through unary_promote_dtypes->coerce_dtype(arr,'float32') dropping imag, then re-tags complex. fr.mean(complex)->real (match_dtype_float! in stats.rs) same corruption. Add a complex branch dispatching to the _complex transcendentals BEFORE the f32/f64 coercion. (#921)
- Blocker for R-1 of ferray-core-complex: fr.array([1+2j])/asarray(np_complex) reject complex — match_dtype_all! (conv.rs) + the array/asarray copy path (creation.rs) are real-only (11 dtypes, no Complex32/Complex64 arm). Add complex arms so input coercion accepts complex128/64, mirroring ma #868. (#920)
- Bump workspace version 0.4.0 -> 0.4.1 to publish sin_fast/cos_fast (#761) (#762)
...
### Added
- fr (main array) structured (void) + object dtype support — fr.array([(1,2.0)],dtype=[('a','i4'),('b','f4')])/array(object) reject; numpy constructs structured/object arrays. Detect-and-delegate construction + manipulation passthrough (no arithmetic; structured ops via fr.rec). Last flexible-dtype gaps after complex/datetime/f16/string (#964)
- Feature gap: fr.partition/argpartition are 1-D only (PyReadonlyArray1, no axis param); numpy supports N-D with default axis=-1 + kth array. fr.partition([[3,1,2]],1) -> TypeError. Add N-D + axis param (numpy-delegate or extend) (#963)
...
### Fixed
- Divergence: fr.unique(float array with NaN) returns [nan,nan,1.] (NaN not collapsed/dedup'd, not sorted to end); numpy returns [1., nan] (NaN collapsed to one, sorted last). ferray-stats unique_values NaN handling. equal_nan=True default collapses NaN (#977)
- Divergence: fr.roll(tuple shift, axis=tuple) raises TypeError (multi-axis roll); fr.unique(return_counts=/return_index=/return_inverse=) raise TypeError (numpy returns tuple). Add roll multi-axis + unique return_* kwargs (#976)
- Divergence: top-level reduction kwargs missing — fr.sum/prod(where=), fr.mean/sum(dtype=), fr.std/var(keepdims= with axis), fr.argmax/argmin(keepdims=) raise TypeError; numpy supports where/dtype/keepdims/initial on reductions. Add the kwargs (numpy-delegate when present) (#975)
- Divergence: fr.searchsorted(a, SCALAR v) raises TypeError (array v works); numpy returns a scalar index. fr.linalg.norm lacks axis=/ord= kwargs (norm(2d,axis=1)->TypeError, numpy [5,13]). Add searchsorted scalar-v + norm axis/ord (#974)
- Divergence: fr.pad reflect mode WRONG (pad([1,2,3,4],2,'reflect')->[2,1,1,2,3,4,3,2] vs numpy [3,2,1,2,3,4,3,2] — repeats edge); modes linear_ramp/maximum/mean/minimum/median MISSING (TypeError). symmetric/edge/wrap/constant OK. Fix reflect + add missing modes (delegate to numpy.pad) (#973)
- Divergence: fr.histogram2d rejects int bins (signature) + returns int counts not float64 + wrong default bins; fr.histogramdd missing required arg; fr.outer rejects int input (unsupported dtype for float, numpy outer(int)->int); fr.kron(1-D complex) ValueError (numpy computes). Binding signature/dtype gaps in histogram2d/histogramdd/outer/kron-1d (#969)
- Divergence (R-CODE-4): fr.cov/corrcoef(complex) SILENTLY drop imaginary (real result); fr.nan_to_num(complex) does NOT replace nan (returns nan+nanj vs numpy 0+0j). numpy cov/corrcoef complex use conj; nan_to_num replaces complex re/im nan. Complex data corruption in cov/corrcoef/nan_to_num (#968)
- Divergence: fr.equal/not_equal(object/structured array) reject (TypeError); numpy computes object equality (np.equal(object_arr,object_arr)->bool) and raises UFuncTypeError for structured. Comparison ufunc gate is string-only (kind U/S), needs object(O) compute + structured(V) numpy-faithful exception (#967)
- Divergence: fr.gradient is 1-D only (rejects 2-D/N-D arrays: 'ndarray not instance of ndarray'); numpy gradient supports N-D returning a list of per-axis gradients + varargs spacing + axis/edge_order. fr.gradient(2d)->TypeError (#966)
- Divergence (R-CODE-4): fr.einsum(complex operands) SILENTLY discards imaginary part — einsum('ij,jk->ik',complex,complex) returns real result. numpy computes complex. einsum lacks complex dispatch (data corruption) (#965)
- Divergence: fr.broadcast_to (stride_tricks.rs) rejects datetime64/timedelta64/float16/string(U,S)/complex dtypes; numpy broadcasts all. Recurring spillover across dtype epics #919/#940/#951/#958 — broadcast_to lacks the detect-and-delegate arms the manipulation ops got. Add datetime/f16/string/complex passthrough (#962)
- Divergence: fr.nansum(int array) returns float64; numpy returns int64/uint64 (nansum preserves int dtype like sum #858). bind_nan_reduction! default f64 coercion. Follow-up to #946/#950 (nanmin/nanmax fixed; nansum int still widens) (#957)
- Divergence: fr.median/percentile/quantile/nanmin/nanmax/nansum/nanmean/nanstd/nanvar/nanmedian/average widen float16 input to float64 output; numpy preserves float16. Follow-up to #954 (sum/mean/std f16 fixed; nan/quantile variants still widen). Add f16 numpy-delegate arms (#956)
- Divergence: fr.nanmin/nanmax/nanmean(int array) return float64; numpy returns int64 (nanmin/nanmax preserve int dtype; nanmean->float64 is correct). bind_nan_reduction! macro coerces int->f64. Affects nanmin/nanmax (nansum already #946) (#950)
- Divergence: fr.sort/argsort/partition/argpartition default axis=None (flatten); numpy defaults axis=-1 (last axis). fr.sort([[3,1],[2,4]])->[1,2,3,4] vs numpy [[1,3],[2,4]]. Affects all 2-D+ sort. (sum/mean/cumsum correctly default axis=None — only sort family wrong) (#949)
- Divergence: fr.maximum/minimum(complex) raise TypeError (numpy computes lexicographic max/min via complex-ordering loop); fr.fmax/fmin(complex) SILENTLY discard imaginary part (data corruption R-CODE-4). numpy: maximum([1+2j,3+1j],[1+5j,3-1j])=[1+5j,3+1j] lexicographic (#929)
- Divergence: fr.sort with NaN broken — sort([3,nan,1]) returns [3,nan,1] unsorted; numpy puts NaN last -> [1,3,nan]. NaN-aware comparator needed (NaN sorts to end) (#918)
