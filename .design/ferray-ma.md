...
- REQ-29: masked covariance/correlation `numpy.ma.cov`/`corrcoef` for the `y=None` case (`numpy/ma/extras.py:1580`/`:1675`): each variable centered by its own masked mean, pairwise normalised by the count of jointly-unmasked observations (`fact = notmask·notmaskᵀ − ddof`), entry masked where `fact ≤ 0`; corrcoef divides by the outer product of the per-variable stddevs. SHIPPED — `ma_cov`/`ma_corrcoef` in `extras.rs`; non-test production consumers `ferray-python/src/ma.rs::cov`/`corrcoef`. The two-variable-set `y` argument remains a ferray-ma library gap (binding raises a clear `ValueError`).
  - **1-D-input shape (closes #1071):** a 1-D input is a SINGLE observation-vector. `numpy.ma.cov` returns the scalar variance shape `()` (`numpy/ma/extras.py:1580` → `numpy.cov` scalar), and `numpy.ma.corrcoef` of one variable degenerates to the `masked` singleton, shape `()`. SHIPPED — the binding `cov`/`corrcoef` (`ferray-python/src/ma.rs`) detect `m.ndim() <= 1` and collapse the `(1,1)` kernel matrix to a 0-d scalar `MaskedArray` (`ma_cov_1d_scalar`) / return `ma_masked_singleton` respectively; non-test consumers are the `#[pyfunction]` entry points themselves, registered via `register_ma_module` (`ferray-python/src/lib.rs`).
...
- REQ-32: `numpy.ma.notmasked_contiguous`/`notmasked_edges` (`numpy/ma/extras.py:1977`/`:1925`) — `axis=None` delegates to the flat forms; 2-D `notmasked_contiguous_axis` returns one slice-list per lane along the orthogonal axis. SHIPPED — `notmasked_contiguous_axis`/`notmasked_edges` in `extras.rs`; non-test production consumers `ferray-python/src/ma.rs::notmasked_contiguous`/`notmasked_edges`.
  - **multi-axis edge-coordinate form (closes #1072):** `numpy.ma.notmasked_edges(a, axis=k)` on a 2-D array returns `[(rows, cols)_first, (rows, cols)_last]` — per-lane first/last unmasked coordinate tuples, compressing fully-masked lanes (`numpy/ma/extras.py:1971-1974`). SHIPPED — `notmasked_edges_axis2` on `MaskedArray<T, Ix2>` in `extras.rs`; non-test production consumer `ferray-python/src/ma.rs::notmasked_edges` (the explicit-axis branch, which builds the numpy `(rows, cols)` int tuples via `edge_coord_tuple`). The former `ValueError` gap is now closed.
...
### REQ status (this iteration — median `axis=` / average `returned=` kwargs, refs #835; closes #1073)

| REQ | Status | Evidence (impl + non-test production consumer) |
| --- | --- | --- |
| REQ-38 (median axis=) | SHIPPED | `numpy.ma.median(a, axis=k)` (`numpy/ma/extras.py:678`) — per-lane median of the unmasked elements along `axis`, all-masked lanes masked. Library: `ma_median_axis` in `extras.rs` (strided lane sweep, even/odd interpolation matching the flat `MaskedArray::median`). Non-test production consumer: the explicit-axis branch of `ferray-python/src/ma.rs::median`, registered via `register_ma_module` (`ferray-python/src/lib.rs`); `keepdims=True` is delegated to numpy.ma for the kept-dim shape. |
| REQ-39 (average returned=) | SHIPPED | `numpy.ma.average(..., returned=True)` (`numpy/ma/extras.py:510`, `return avg, scl` at `:670`) — the `(average, sum_of_weights)` tuple; `sum_of_weights` is the applied (unmasked) weight total, or the count of unmasked elements for the unweighted case (`numpy/ma/extras.py:637`). Library: `MaskedArray::average_returned` in `extras.rs` (the existing `average` now delegates to it). Non-test production consumer: `ferray-python/src/ma.rs::average` (the `returned` branch builds the Python 2-tuple), registered via `register_ma_module` (`ferray-python/src/lib.rs`). |
