# Feature: ferray-polynomial — Complete numpy.polynomial with all basis classes
...
### Root Finding
- REQ-12: `roots()` computes roots via companion matrix eigenvalues (delegates to `ferray-linalg::eigvals`). Returns all roots including complex ones.
...
- [ ] AC-6: `cargo test -p ferray-polynomial` passes. `cargo clippy` clean.
