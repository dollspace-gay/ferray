...
### Python Class-API numpy Parity (Section 11.3 — `ferray-python` wrappers)

The six `#[pyclass]` wrappers (`ferray.polynomial.Polynomial` / `Chebyshev` /
`Hermite` / `HermiteE` / `Laguerre` / `Legendre`, generated by the
`poly_class!` macro in `ferray-python/src/polynomial.rs`) carry numpy's
`numpy.polynomial._polybase.ABCPolyBase` surface. The Rust math
(`ferray-polynomial`) operates in raw window-space coefficients; the
domain->window affine Jacobian is a CLASS-LEVEL concern applied in the macro
exactly as `numpy/polynomial/_polybase.py` does.

REQ status table (impl `file:line` + non-test production-consumer `file:line`):

| REQ | Status | Implementation | Production consumer |
|-----|--------|----------------|---------------------|
| REQ-PY-1: `roots()`/`deriv()`/`integ()` apply the domain->window Jacobian (`_polybase.py:878,845,900,913`) | SHIPPED | `ferray-python/src/polynomial.rs` macro `deriv`/`integ`/`roots` (~`:534-600`) + `map_roots_window_to_domain` (`:445`) | `ferray-python/src/lib.rs:253-258` (`add_class::<polynomial::Polynomial>()` … registers the classes into `ferray.polynomial`) |
| REQ-PY-2: arithmetic dunders `__neg__`/`__add__`(+scalar)/`__sub__`/`__mul__`(+scalar)/`__floordiv__`/`__mod__`/`__eq__` (`_polybase.py:522,530,546,565,571,643`) | SHIPPED | `ferray-python/src/polynomial.rs` macro dunder block + `get_coefficients` helper (`:404`) | `ferray-python/src/lib.rs:253-258` |
| REQ-PY-3: core methods `convert(kind=)`/`fromroots`/`basis`/`identity`/`mapparms`/`cutdeg`/`linspace` (`_polybase.py:779,1037,1115,1080,816,704,915`) | SHIPPED | `ferray-python/src/polynomial.rs` macro methods + `build_from_power_for_kind` dispatcher (end of file) | `ferray-python/src/lib.rs:253-258` |
| REQ-PY-4: kwargs `integ(lbnd=)` + `fit(domain=/window=/full=/rcond=/w=)` (`_polybase.py:845,946`) | SHIPPED | `ferray-python/src/polynomial.rs` macro `integ`/`fit` signatures | `ferray-python/src/lib.rs:253-258` |

Verified against the installed numpy 2.4.x oracle:
`ferray-python/tests/divergence_polynomial.py` (34 pins green) +
`ferray-python/tests/test_polynomial_classes.py` (74 green).
