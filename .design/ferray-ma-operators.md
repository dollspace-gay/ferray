...
All REQs are NOT-STARTED: there are NO operator dunders in `ferray-python/src/ma.rs`
(confirmed: zero `__add__`/`__sub__`/`__mul__`/`__truediv__`/`__richcmp__`/`__eq__`/
`__neg__`/`__abs__`/`__invert__`/`__and__`/`__lshift__`/… in the file), and the live
binding raises `TypeError` for all operators (`a == 2` returns Python `False`).

| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (R-A arithmetic) | NOT-STARTED | open prereq blocker #862. No `__add__`/`__sub__`/`__mul__`/`__truediv__`/`__floordiv__`/`__mod__`/`__pow__` in `ma.rs` (grep returns nothing); live `a+1`→`TypeError: unsupported operand type(s) for +`. Compute backend EXISTS: `pub fn add`/`subtract`/`multiply`/`divide`/`floor_divide`/`power`/`mod_` in `arithmetic.rs`. Gap is purely the binding dunders + dtype-generic mask propagation. |
| REQ-2 (R-B comparison) | NOT-STARTED | open prereq blocker #863. No `__lt__`/`__le__`/`__gt__`/`__ge__` in `ma.rs`; live `a>2`→`TypeError: '>' not supported`. Compute backend EXISTS: `pub fn less`/`less_equal`/`greater`/`greater_equal` in `comparison.rs` (`-> Array<bool, D>`). |
| REQ-3 (R-C bitwise) | NOT-STARTED | open prereq blocker #865. No `__and__`/`__or__`/`__xor__`/`__invert__`/`__lshift__`/`__rshift__` in `ma.rs`; live `~a`→`TypeError: bad operand type for unary ~`. Compute backend EXISTS: `pub fn bitwise_and`/`bitwise_or`/`bitwise_xor`/`invert`/`left_shift`/`right_shift` in `bitwise.rs`, `BitwiseOps`/`ShiftOps` over int/bool. |
...
| REQ-6 (R-F reflected + scalar) | NOT-STARTED | open prereq blocker #862 (folded into R-A: reflected dunders + scalar coercion ship with the arithmetic dunders). Coercion machinery EXISTS: `coerce_scalar in ma.rs` (routes a scalar through `numpy.asarray(value, dtype)` to the native dtype) and `coerce_to_ma in ma.rs` (operand → masked array). |
| REQ-7 (R-G `__eq__`/`__ne__`) | NOT-STARTED | open prereq blocker #863 (folded into R-B). Live `a==2` returns Python `bool False` (object-identity fallback — no `__eq__` in `ma.rs`); numpy returns a masked bool array. Needs the `_comparison` eq/ne special-case (`numpy/ma/core.py:4245`): where masked-in-both → "equal", masked-in-one → "unequal". |
