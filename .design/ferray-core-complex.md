...
Every REQ is **NOT-STARTED**: the binding dispatch sites are real-only today
and the divergences reproduce live (numpy 2.4.5). The library substrate behind
each is largely already shipped (by the ma-complex epic), so each blocker is
binding-dispatch threading, not net-new library infrastructure — but until that
threading lands and a pinned pytest goes green, the REQ stays NOT-STARTED
(R-DEFER-2, two states only).

| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (input coercion) | NOT-STARTED | open prereq blocker #920. `fr.array`/`asarray` (`pub fn array` / `pub fn asarray` in `creation.rs`) round-trip the buffer through `match_dtype_all!` (`conv.rs`), an 11-real-dtype macro whose fallthrough is `PyTypeError "unsupported dtype: {other:?}"` — no `Complex32`/`Complex64` arm. Live: `fr.array([1+2j])` → `TypeError: unsupported dtype: "complex128"`. Library is ready (`Element for Complex`, `creation_dispatch!` complex arms already SHIPPED for `zeros`). Fix = add complex arms (or route complex through `complex_ferray_to_pyarray`) in the `array` copy path, mirroring ma #868. |
| REQ-2 (STOP imag-discard) | NOT-STARTED | open prereq blocker #921. `unary_float_body!` (`ufunc.rs`) calls `unary_promote_dtypes` then `coerce_dtype($py, &$arr, compute_dt)` with `compute_dt = "float32"` for a complex input (complex is not in the float-passthrough set, so it falls to `result_type(sample, float16)` → `complex128`, and `compute = "float32"` since `out_name != "float64"`), which numpy casts complex→float32 DROPPING the imaginary part; the result is then re-tagged complex. `pub fn mean` (`stats.rs`) uses `match_dtype_float!`, coercing complex→f64. Live: `fr.sqrt(z)` → `[1.+0.j]` + `ComplexWarning`; `fr.mean(z)` → `1.0`. Fix = a complex branch dispatching to the `_complex` ops BEFORE the f32/f64 coercion (satisfied jointly by REQ-3/REQ-6). |
| REQ-3 (complex transcendentals) | NOT-STARTED | open prereq blocker #922. The library functions ALL EXIST: `ferray_ufunc::{sqrt_complex, exp_complex, ln_complex, sin_complex, …}` (`ops/complex.rs` `complex_unary!` macro + `power_complex`), re-exported at the crate root (`ferray-ufunc/src/lib.rs`). The gap is purely the binding: `bind_unary_float!`/`unary_float_body!` (`ufunc.rs`) have no complex arm. Fix = a complex dispatch branch in the unary-float bind path calling the `_complex` op for a complex dtype. NO net-new library work. |
