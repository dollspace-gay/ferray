...
| REQ-3 (comparison) | SHIPPED | #961. `less`/`greater`/`less_equal`/`greater_equal`/`equal`/`not_equal` generated by `bind_comparison!` (`ufunc.rs`) gained an `is_string_compare` branch (both operands' `np.asarray(x).dtype.kind ∈ {U,S}`, reusing `crate::manipulation::is_string_array`) AHEAD of the float16 / real / complex paths in BOTH macro arms (equality + ordering), delegating to `compare_string` → `np.<op>(x1, x2)` on the ORIGINAL operands (bool output, codepoint-lexicographic, broadcast, width-independent — no transport, strings never enter the Rust library, R-CODE-4). Mirrors the datetime `is_time_compare`/`compare_time` seam minus the int64-view round-trip. Mixed `'U'`-vs-`'S'` delegates too, so numpy raises its own `UFuncTypeError` (matching numpy, not ferray's old `unsupported dtype`). Non-test consumer: the `less`/`greater`/`equal`/`not_equal`/`greater_equal`/`less_equal` `#[pyfunction]`s registered top-level in `lib.rs` `_ferray`. numpy (live 2.4.4): `np.less(['abc','xyz','abc'],['abd','abc','abc'])` → `[True,False,False]`; `np.equal(['a','b'],'a')` → `[True,False]` (broadcast); `<U2`-vs-`<U3` compares by value; `np.equal(<U,<S)` → `UFuncTypeError`. Pinned green: `tests/test_expansion_string_compare.py` (83 cases — all six ops × array/array, broadcast scalar both sides, different widths, bytes `<S`, empty strings, non-ASCII, 2-D, empty array, str-scalar pair, mixed `U`-vs-`S` matches numpy's raise; numeric/float/datetime real paths unregressed). |
...
numpy's result directly. **All prereq blockers closed** (#959, #960, #961); R-4
already shipped — the whole string main-array surface now works, **epic #958
CONVERGED**. Biggest risk: numpy's flexible-dtype edge cases —
