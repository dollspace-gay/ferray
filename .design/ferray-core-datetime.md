...
| REQ-3 (comparison + ordering) | SHIPPED | #943. The comparison + ordering surfaces now compute by int64 TICK value over the existing int64-view transport (#831), promoting same-kind operands to a common finer unit via numpy's own `promote_types`+`astype`. **Comparison** (`fr.less`/`greater`/`less_equal`/`greater_equal`/`equal`/`not_equal`): `enum TimeCompare` + `fn compare_time` + `fn is_time_compare` (`datetime.rs`) compute bool with numpy's NaT-unordered rule (`cmp_ticks`: any NaT operand → all-False except `!=`→True). **Ordering** (`fr.sort`/`argsort`/`unique`/`searchsorted`/`partition`/`argpartition`): `fn order_key` maps NaT (`i64::MIN`)→`i64::MAX` so an ascending int64 sort places NaT LAST (like NaN); `fn sort_time`/`argsort_time` (axis-aware, stable), `fn unique_time` (NaT collapsed to one), `fn searchsorted_time` (binary search, left/right, cross-unit promote), `fn partition_time`/`argpartition_time` (full ordered copy satisfies the partition postcondition) — datetime/timedelta re-tagged with the input unit; argsort/argpartition/searchsorted → int64; compares → bool. Non-test consumers: the `equal`/`not_equal`/`less`/`less_equal`/`greater`/`greater_equal` `#[pyfunction]`s (`ufunc.rs`, `bind_comparison!` macro — each branches on `crate::datetime::is_time_compare` ahead of its real/complex path) and the `sort`/`argsort`/`unique`/`searchsorted`/`partition`/`argpartition` `#[pyfunction]`s (`stats.rs` — each branches on `crate::datetime::is_time_array`), all registered top-level in `lib.rs`. numpy 2.4 live: `a<a[0]→[False True False]`; `np.sort(['01-03','01-01','01-05'])→['01-01','01-03','01-05']`; `np.argsort→[1 0 2]`; `np.sort([dt,NaT,dt])` puts NaT last; `NaT<x`/`NaT==NaT`→False, `NaT!=NaT`→True; cross-unit `dt[D]==dt[h]` promotes; `np.unique` collapses multiple NaT to one trailing NaT; `np.searchsorted(sorted,q,side)`→int64. Verification: `tests/test_expansion_datetime_order.py` (67 cases — compare datetime/timedelta/cross-unit/NaT/broadcast, sort/argsort/unique/searchsorted/partition/argpartition datetime+timedelta, NaT-last, real+complex unregressed; all live-derived, R-CHAR-3). |
...
| REQ-5 (arange datetime) | SHIPPED | #945. `arange` (`creation.rs`, `pub fn arange`) branches to `arange_time` (`creation.rs`) when `dtype` is datetime64/timedelta64 (`crate::datetime::is_time_dtype_name`) OR a start/stop/step operand is a datetime64/timedelta64 scalar/array (`crate::datetime::any_time_operand`, `datetime.rs`) — AHEAD of the f64-coercion path that raised `TypeError: must be real number, not str`. `arange_time` delegates the range to numpy (`numpy.arange(start[, stop[, step]], dtype=dtype)` on the ORIGINAL operands — numpy owns the string→datetime64 parse, calendar month/year stepping, int/timedelta64 step, and half-open `[start,stop)` semantics) then marshals the datetime64/timedelta64 buffer back through the int64-view transport (`crate::datetime::datetime_roundtrip`, #831/#941), preserving unit+shape+NaT with no lossy cast (R-CODE-4). Non-test consumer: the `arange` `#[pyfunction]` registered top-level in `lib.rs`. numpy 2.4.5 live: `np.arange('2020-01','2020-04',dtype='datetime64[M]')==['2020-01','2020-02','2020-03']`; `np.arange(d0,d1,np.timedelta64(2,'D'))` steps by the timedelta; `np.arange(np.datetime64('2020-01-01'),np.datetime64('2020-01-10'),2,dtype='datetime64[D]')` int-step; `np.arange(5,dtype='timedelta64[D]')==[0,1,2,3,4]`; `np.arange('2020-04','2020-01',dtype='datetime64[M]')==[]` (empty). Verification: `tests/test_expansion_datetime_arange.py` (live-derived, R-CHAR-3). |
