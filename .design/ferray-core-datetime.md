...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (input coercion) | NOT-STARTED | open prereq blocker #941. `match_dtype_all!` (`conv.rs`) and `creation_dispatch!` (`creation.rs`) have no `"M"`/`"m"` arms → `other =>` raises `TypeError: unsupported dtype: "datetime64[D]"`. Transport (`as_int64_view`/`int64_to_datetime64` in `conv.rs`, #831) + storage (`DynArray::zeros(DType::DateTime64(unit),…)` in `dynarray.rs`) already exist; only the binding dispatch arm is missing. Live: `fr.array(['2020-01-01'],dtype='datetime64[D]')` → `TypeError`. |
| REQ-2 (datetime arithmetic) | NOT-STARTED | open prereq blocker #942. The ± cases SHIPPED: `pub fn add`/`subtract` in `ufunc.rs` branch on `crate::datetime::is_time_op` → `add_time`/`subtract_time` (`datetime.rs`) → `add_datetime_timedelta_promoted`/`sub_datetime_promoted`/`add_timedelta_promoted` (`ops/datetime.rs`). GAP: `timedelta*int`/`/`/`//`/`%` hit `match_dtype_numeric` → `TypeError: unsupported dtype for numeric op` (no kernel exists); `datetime+datetime`/`datetime*scalar` raise ferray's text not numpy's `UFuncTypeError`. Needs `multiply_time`/`divide_time` binding arms + four small `ops/datetime.rs` kernels + raise-text alignment. Live: `fr.multiply(td,2)` → `TypeError unsupported dtype for numeric op`. |
...
| REQ-4 (reductions — STOP divergence) | NOT-STARTED | open prereq blocker #944. **Active divergence:** `fr.mean(dt)` returns `array(18268.5)` (raw float, drops the datetime contract) where numpy RAISES `UFuncTypeError: ufunc 'add' cannot use operands … <M8[D]>`; `fr.std(td)`/`fr.var(td)` return floats where numpy RAISES `TypeError: ufunc 'square' not supported`. `fr.sum(td)` → `unsupported dtype` where numpy → `timedelta64`. Fix = `stats.rs` `"M"`/`"m"` arms computing the computable reductions (`sum`/`mean` timedelta→timedelta, `ptp` datetime→timedelta) over int64 ticks + re-tag, and RAISING numpy's exact exception for the non-computable ones (mean/sum/std/var datetime; std/var timedelta). Live oracle: `np.mean(dt)` raises; `np.sum(td)==timedelta64(7,'D')`; `np.ptp(dt)==timedelta64(5,'D')`. |
