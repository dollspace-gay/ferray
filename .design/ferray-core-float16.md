...
R-3 (binary arithmetic + NEP-50 promotion) is now **SHIPPED** too (#953 — see
...
| REQ-3 (arithmetic + NEP-50) | SHIPPED | `fn binary_involves_float16` + `fn f16_binary_delegate` (`ufunc.rs`, #953) detect a float16 operand (`dtype_name` on either `as_ndarray`'d input) and delegate the op to numpy's own top-level function (`np.<func>(x1,x2)`) on the ORIGINAL operands, so numpy owns BOTH the NEP-50 promotion (`result_type`) and the float16 compute contract (f32-compute + f16-narrow, overflow→inf). The f16 guard is wired AHEAD of the real-only `match_dtype_numeric!` / `match_dtype_float_or_int!` funnel in the consumers `pub fn add`/`subtract`/`multiply`/`divide` (the `binary_numeric_body!` ops) and `pub fn power`/`maximum`/`minimum` (the `bind_binary_numeric_split!` `f16 =` macro arms) and `pub fn floor_divide`/`remainder`/`mod_` (`ufunc.rs`), all registered top-level in `lib.rs`. Same detect-and-delegate pattern as the float16 reductions (`crate::stats::f16_reduce`, #954) and creation coercion (#952), keeping `half::f16` out of the Rust boundary. numpy (live 2.4.4): `np.add(f16,f16).dtype == float16`, `f16+f32 → float32`, `f16+f64 → float64`, `f16+pyfloat → float16`, `f16+int32-array → float64`, `np.maximum/floor_divide/remainder/power(f16,f16).dtype == float16`, `np.add([60000,60000]f16) == inf`. Pinned green: `tests/test_expansion_float16_arith.py` (59 tests — 10-op f16+f16 parametrize, NEP-50 mixed promotion, per-op power/floor_divide/remainder/min-max/overflow, scalar collapse, real-path regression guards). |
...
| REQ-5 (comparison + manipulation) | SHIPPED | A shared detect-and-delegate seam — `pub fn is_float16_dtype` + `pub fn f16_delegate` (+ `f16_delegate_list` for split ops) in `conv.rs` (#955) — delegates any float16-operand op to numpy's own top-level function on the ORIGINAL operand(s), returning numpy's result directly (since `fr.ndarray` IS `numpy.ndarray`, no marshalling round-trip is needed). The seam is wired AHEAD of each real-only dispatch macro: comparisons `pub fn less`/`greater`/`less_equal`/`greater_equal`/`equal`/`not_equal` and the unary numeric/predicate ops `sign`/`absolute`/`negative`/`floor`/`ceil`/`trunc`/`round`/`square`/`reciprocal`/`positive`/`around`/`isnan`/`isinf`/`isfinite`/`signbit`/`ediff1d` (`ufunc.rs`, via `binary_involves_float16` + the `bind_*`/`comparison`/predicate macro arms); the data-move / view ops `reshape`/`ravel`/`flatten`/`squeeze`/`expand_dims`/`transpose`/`swapaxes`/`moveaxis`/`rollaxis`/`flip`/`fliplr`/`flipud`/`rot90`/`roll`/`atleast_*`/`broadcast_to`/`concatenate`/`stack`/`vstack`/`hstack`/`dstack`/`column_stack`/`block`/`pad`/`tile`/`repeat`/`delete`/`insert`/`append`/`resize`/`split`/`array_split`/`vsplit`/`hsplit`/`dsplit` (`manipulation.rs`, each twin to its datetime `delegate_manip` branch); the order/set/diff ops `sort`/`argsort`/`searchsorted`/`unique`/`unique_extended`/`count_nonzero`/`diff`/`cumprod`/`partition`/`argpartition`/`lexsort`/`argmin`/`argmax`/`intersect1d`/`union1d`/`setdiff1d`/`setxor1d`/`isin`/`in1d`/`where_fn` (`stats.rs`); and the gather ops `flatnonzero`/`nonzero`/`take`/`take_along_axis`/`choose`/`compress`/`select`/`extract`/`argwhere`/`place`/`putmask`/`put` (`indexing.rs`). numpy registers a float16 compare loop (generate_umath.py:567 `less` `TD(inexact + times, out='?')`, `inexact` includes `'e'`/float16) → bool, and preserves float16 for every data-move op. Non-test consumers: all the listed top-level `#[pyfunction]`s, registered in `lib.rs`. numpy (live 2.4.4): `np.less(f16,f16)` → bool, `np.reshape/transpose/sort/unique/where/clip/round/concatenate/diff(f16).dtype == float16`. Pinned green: `tests/test_expansion_float16_ops.py` (70 tests — comparison→bool, unary f16-preserve, predicate→bool, data-move/set/gather f16-preserve, square-overflow→inf, sort NaN-last, real f32/f64/int regression guards). NOTE (spillover, separate blocker): the nan-aware / quantile reductions `median`/`percentile`/`quantile`/`nanmin`/`nanmax`/`nansum`/`nanmean`/`nanstd`/`nanvar`/`nanmedian`/`average` still WIDEN a float16 input's output to float64 (numpy preserves float16) — a distinct R-DEV-3 widening (not a rejection), tracked as a follow-up under epic #951. |
