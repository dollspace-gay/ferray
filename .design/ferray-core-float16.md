...
Verified live (numpy 2.4.5, baseline `0a558feb`):
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (input coercion) | NOT-STARTED | open prereq blocker #952. `fr.zeros`/`ones`/`empty`/`*_like(dtype='float16')` ARE SHIPPED — the `"float16" \| "f16"` arm in `macro_rules! creation_dispatch` (`creation.rs`) builds the values as `ArrayD<f32>` via ferray-core then `f32_any.call_method1("astype", ("float16",))`, consumed by `pub fn zeros`/`ones`/`empty` + the `*_like` fns (registered top-level in `lib.rs`); pinned green `test_zeros_float16_dtype_supported`, `test_zeros_like_preserves_float16_dtype`. BUT `pub fn array`/`pub fn asarray` (`creation.rs`) dispatch `match_dtype_all!` (`conv.rs`, 11 real arms, no float16), and `full_impl` (`creation.rs`) has no float16 arm — so `fr.array([1.,2,3],dtype='float16')` / `fr.asarray(np.array(...,'float16'))` / `fr.full(3,2.0,dtype='float16')` raise `TypeError: unsupported dtype: "float16"` (live, numpy 2.4.5: `np.array([1.,2,3],dtype='float16').dtype == float16`). Fix = a float16 branch ahead of the macro routing through `np.asarray(obj,'float16')` (numpy owns the f32→f16 narrowing), mirroring the SHIPPED creation arm. |
...
| REQ-4 (reductions — STOP widening) | NOT-STARTED | open prereq blocker #954. `fr.sum`/`prod`/`cumsum`/`cumprod` dispatch `match_dtype_numeric!` (`stats.rs`) → `TypeError: unsupported dtype for numeric op: "float16"`; `fr.mean`/`var`/`std`/`average` coerce the input to `float64` ahead of `match_dtype_float!` (`stats.rs` module doc, "mean/var/std/average coerced the input to float64/f32"), so `fr.mean(np.array([1,2,3],dtype='float16'))` returns `array(2.)` **dtype float64** where numpy returns dtype **float16** — an active silent-widening divergence (R-DEV-3 / R-CODE-4). numpy 2.4.5 live: `np.sum/mean/prod/min/max/std/var(f16).dtype == float16`. Fix = float16 arms in `stats.rs` that compute at f32 and narrow the reduced result to float16 via `astype` (and for `mean`/`std`/`var`, stop coercing the *output* to float64). |
