...
- [ ] AC-12 (REQ-21): `type(fr.sum([1,2,3])) is numpy.int64` (NOT `numpy.ndarray`); `isinstance(fr.sum([1,2,3]), numpy.ndarray) is False`; `type(fr.mean([1.,2,3])) is numpy.float64`; `type(fr.all([True,True])) is numpy.bool_`; while `fr.sum([[1,2],[3,4]], axis=0)` stays a 1-D `numpy.ndarray` (`array([4,6])`).

## REQ status table

| REQ | Status | Evidence |
|---|---|---|
| REQ-21 (reduction full-reduction returns numpy scalar, not 0-d ndarray) | NOT-STARTED | open prereq blocker #992. The reduction `#[pyfunction]` egresses in `stats.rs` end their native compute path with `r.into_pyarray(py)…into_any()` (or `u64_arrd_to_i64_pyarray`), which produces a **0-d `numpy.ndarray`** for a full reduction rather than the numpy scalar numpy's `$OUT_SCALAR` contract requires. Quoted evidence — `pub fn sum` in `stats.rs` ends with `r.into_pyarray(py).map_err(ferr_to_pyerr)?.into_any()`; `pub fn all` in `stats.rs` builds a 0-d array (`ArrayD::<bool>::from_vec(IxDyn::new(&[]), vec![r])…into_pyarray(py)…into_any()`) for the `axis=None` branch; the `bind_nan_reduction!` macro body in `stats.rs` likewise emits `r.into_pyarray(py).map_err(ferr_to_pyerr)?.into_any()`. None of these route the 0-d result through the existing `pub fn scalarize` in `conv.rs` (which returns `result[()]` when `result.ndim == 0` and is a no-op otherwise — already the intended mechanism, used by the ufunc shim via `pub fn all_scalar_inputs` in `conv.rs`). The fix the blocker tracks: route every in-scope full-reduction (0-d) egress through `conv::scalarize` so `numpy.int64`/`numpy.float64`/`numpy.bool_`/`numpy.complex128` is returned, leaving ≥1-D axis results as `ndarray`. Mirrors numpy/\_core/fromnumeric.py:83 (`return ufunc.reduce(...)`) and the established `complex.rs`/`matrix_rank`/`trapezoid`/`linspace(retstep=)` scalar-collapse precedent in `tooling/translate-routes.toml`. |
