...
| REQ-D5 (datetime64/timedelta64 dtype-passthrough manipulation, #948) | SHIPPED | The manipulation surface (`reshape`/`ravel`/`flatten`/`squeeze`/`expand_dims`/`transpose`/`swapaxes`/`moveaxis`/`rollaxis`/`flip`/`fliplr`/`flipud`/`rot90`/`roll`/`atleast_1d`/`atleast_2d`/`atleast_3d`/`tile`/`repeat`/`delete`/`insert`/`append`/`resize`/`pad`/`block`/`concatenate`/`stack`/`vstack`/`hstack`/`column_stack`/`dstack`/`split`/`array_split`/`vsplit`/`hsplit`/`dsplit` in `ferray-python/src/manipulation.rs`) branches on `crate::datetime::is_time_array` ahead of the real-only `match_dtype_all_complex!` (which raised `TypeError: unsupported dtype "datetime64[D]"`). These ops carry NO element arithmetic — pure data moves — so each delegates to numpy's own op on the coerced time array (numpy owns the dtype-passthrough incl. `concatenate`'s cross-unit promotion to the finer unit via `result_type`, `numpy/_core/multiarray.py:198`; `reshape` `numpy/_core/fromnumeric.py:208`; `repeat` `:438`; `transpose` `:605`; `roll` `numpy/_core/numeric.py:1226`; `flip` `numpy/lib/_function_base_impl.py:284`; `stack` `numpy/_core/shape_base.py:379`) and marshals the result back through the int64-view transport `crate::datetime::delegate_manip` / `delegate_manip_list` → `datetime_roundtrip` (#941), preserving dtype+unit+shape+NaT with no lossy cast (R-CODE-4). Non-test consumer: every listed `#[pyfunction]` registered top-level on `_ferray` in `ferray-python/src/lib.rs`. numpy 2.4.5 live: `np.reshape(M8[D],(2,2)).dtype==datetime64[D]`, `np.concatenate([M8[D],M8[s]]).dtype==datetime64[s]`, `np.repeat(m8[D],2)`, NaT preserved. Pinned green: `tests/test_expansion_datetime_manip.py` (54 cases) + the #948 Group-C pins `test_{reshape,transpose,flip,roll,repeat,tile,stack,concatenate}_datetime_preserves_dtype` in `tests/test_divergence_datetime_audit.py`. NOTE: top-level `broadcast_to` is served by `stride_tricks::broadcast_to` (outside this manifest) — its datetime passthrough is a follow-up. |
