// Standalone CGEMM benchmark for ferray-linalg.
// Compares the hand-tuned Complex<f32> path (gemm_c32) against a naive
// reference for correctness and prints GFLOPs/sec at various sizes.
// Each complex multiply = 4 real FLOPs (4 muls + 2 adds + 2 sums for
// add/sub) so we count 8 FLOPs per output element per K iter.
...
fn main() {
    ...
    for &n in &sizes {
        let m = n;
        let k = n;
        let a: Vec<f32> = (0..m * k * 2).map(|i| ((i as f32) * 0.013).sin()).collect();
        let b: Vec<f32> = (0..k * n * 2).map(|i| ((i as f32) * 0.011).cos()).collect();
        ...
        if n <= 128 {
            ...
            naive_cgemm(m, n, k, &a, &b, &mut c_ref);
            for i in 0..m * n * 2 {
