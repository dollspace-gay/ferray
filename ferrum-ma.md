---
title: ferrum-ma — Masked arrays for missing and invalid data
tags: [design-doc]
sources: []
...
- REQ-3: `ma.data()` returns the underlying data array, `ma.mask()` returns the mask ### Masked Reductions
...
- REQ-6: `ma.compressed()` — return a 1D array of unmasked elements only ### Masking Constructors
- REQ-7: `ma::masked_where(&condition, &a)` — mask where condition is true
- REQ-8: `ma::masked_invalid(&a)` — mask NaN and Inf values
- REQ-9: `ma::masked_equal(&a, value)`, `ma::masked_greater(&a, value)`, `ma::masked_less(&a, value)`, `ma::masked_not_equal(&a, value)`, `ma::masked_greater_equal(&a, value)`, `ma::masked_less_equal(&a, value)`, `ma::masked_inside(&a, v1, v2)`, `ma::masked_outside(&a, v1, v2)` ### Masked Arithmetic
...
- REQ-11: Operations between a MaskedArray and a regular array treat the regular array as fully unmasked ### Masked Ufunc Support
- REQ-12: All ferrum-ufunc elementwise operations must accept `MaskedArray` inputs and produce `MaskedArray` outputs, propagating masks correctly. Masked elements are skipped (not computed). ### Masked Sorting
- REQ-13: `ma.sort(axis)` — sort unmasked elements, masked elements move to end
- REQ-14: `ma.argsort(axis)` — return indices that sort unmasked elements ### Mask Manipulation
...
- [ ] AC-6: `cargo test -p ferrum-ma` passes. `cargo clippy` clean.
...
- **Upstream**: `ferrum-core` (NdArray, Element, Dimension, FerrumError), `ferrum-ufunc` (elementwise ops respect masks), `ferrum-stats` (masked reductions)
